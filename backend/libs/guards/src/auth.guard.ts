    ...
    public async canActivate(context: ExecutionContext): Promise<boolean> {
            ...
            try {
                ...
                switch (authType) {
                    case AuthTypeEnum.BEARER:
                        if (await this.bearerGuard.canActivate(context)) { return true }
                        break;
                    case AuthTypeEnum.COOKIE:
                        if (await this.cookieGuard.canActivate(context)) { return true }
                        break;
                ...
                if (typeof error === `object` && error instanceof Error) {
                    ...
                    authAttempts[authType] = new Error(`Unknown error during authentication.`);
                }
        ...
        Object.entries(authAttempts).map(([authType, error]) => {
            this.logger.warn(`${authType} Authentication fail: ${error?.message}`,
                { startTime, tag: LogTypeEnum.AUTHORIZATION_FAIL })
        });

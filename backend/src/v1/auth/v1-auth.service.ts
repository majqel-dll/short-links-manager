...
export class V1AuthService {
    ...
    public async createNewAccount(
        { login, email, password }: SignUpDto
    ): Promise<void> {
        ...
        try {
            ...
            const passwordHash = await argon2.hash(password,
                { type: argon2.argon2id, timeCost: 3, memoryCost: 2 ** 16 });
            ...
            void this.logger.error(`Failed to create new account.`, {
                startTime, tag: LogTypeEnum.CREATE_FAIL, error
            });
    ...
    public async generateAccessToken(
        ...
        const refreshToken = await this.jwtService.signAsync(refreshTokenPayload, {
            secret: process.env.SECRET, expiresIn: refreshTokenExpiresIn
        });

        const accessToken = await this.jwtService.signAsync(tokenPayload, {
            secret: process.env.SECRET, expiresIn: tokenExpiresIn
        });
